package th.co.aware.config;

public class MYKEY {
	public static final String MESSAGE = "key1";
	
	public static final String SES_USERLOGIN = "ses_userlogin";
	public static final String SES_ORDER = "ses_order";
	public static final String SES_ORDERLIST = "ses_orderlist";
	
	public static final String USER_WAITRESS = "Waitress";
	public static final String USER_COOKER = "Cooker";
	public static final String USER_RECIEVEORDERS = "Recieve orders";
	public static final String USER_ADMIN = "Administrator";
}
